...
Evaluation
An example of how I have refactored my code can be seen through my implementation of my revealCell method. Initially, this method only revealed the cell specified in its input, however, I refactored this method to eventually also reveal all adjacent empty cells surrounding the specified cell, in a recursive fashion until cells which are not empty are reached. I have also successfully reused my code on numerous occasions, one of which being my initializeBoard method. The initializeBoard method is reused twice in the newGame method to initialize both the board and displayBoard arrays.
